...
export default function ListLandForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Real backend call — creates an actual row in Postgres, owned by
      // the signed-in user (derived server-side from the JWT; see
      // LandController on the backend). This is the source of truth.
      ...
      // Also recorded in the local ListingsContext "fake backend" (see
      // that file) so it immediately shows up in Explore Land's grid
      // and the dashboard, same as before this integration pass —
      // those browsing pages still read from ListingsContext, not a
      // live GET /api/lands call, so this keeps them working rather
      // than silently going stale. `id` here is the REAL backend id
      // (created.id); everything else stored is the same rich record
      // this form always built, for that local display purpose.
      const localRecord = addLandListing({ ...buildRecord("published"), id: created.id }, "published");
      setPublishedSlug(localRecord.slug);
      setSubmitState("success");
    } catch (err) {
      setSubmitState("error");
      setSubmitErrorMessage(err.message || "Something went wrong publishing your listing. Please try again.");
    }
  }

  function handleSaveDraft() {
    // Drafts only need a title so a listing can be found again later —
    // everything else can stay unfinished. Kept LOCAL-ONLY: the
    // backend's CreateLandRequest has no draft/status concept at all
    // (a created LandListing is always immediately status="ACTIVE",
    // publicly visible via GET /api/lands — see LandController on the
    // backend) — there's no real endpoint a "save as draft" action
    // could call without actually publishing the listing, which would
    // defeat the point of a draft. See FRONTEND_INTEGRATION_NOTES.md
    // "Known gaps".
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-6">
            ...
            {submitState === "error" && (
              <p className="text-center text-sm text-red-600">
                {submitErrorMessage || "Something went wrong publishing your listing. Please try again."}
              </p>
            )}
          </form>
        </div>
