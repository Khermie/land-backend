 ...
 * purpose. Calls the real backend (POST /api/auth/login via
 * AuthContext#login — see context/AuthContext.jsx) and shows whatever
 * message it returns on failure (bad credentials, network issues,
 * etc.) rather than a generic error, since the backend's error message
 * is already meant to be shown directly to the user (see
 * GlobalExceptionHandler on the backend). role/ghanaCardVerified for
 * the redirect after login come from the backend's real response, not
 * a guessed/hardcoded persona.
 */
export default function LoginForm() {
  ...
  // meant to do instead of always landing on their role's default
  // destination.
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(redirectTo || getRoleDestination(toFrontendRole(user.role)));
    } catch (err) {
      // The backend returns every failure (bad credentials, validation,
      // etc.) as HTTP 400 with a real `message` — see services/api.js's
      // ApiError and GlobalExceptionHandler on the backend. Shown
      // directly rather than a generic "something went wrong" since
      // the backend's message is already written to be user-facing.
