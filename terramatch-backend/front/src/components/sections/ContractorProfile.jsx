...
function Breadcrumb({ contractor }) {
  return (
    <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-sm text-ink-500">
      {contractor.breadcrumb.map((crumb) => (
      ...
      <span className="text-ink-900">{contractor.shortName}</span>
    </nav>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-ink-900/10 bg-white p-6">
      <div className="flex items-start gap-4">
        {/*
          PLACEHOLDER ASSET: contractor logo/headshot. Replace with a
          real photo at
          /src/assets/images/contractors/kwame-builders.jpg
        */}
        <div className="relative shrink-0">
          <ImageSkeleton className="h-20 w-20 rounded-full" />
          {contractor.verified && (
  ...
  return (
    <div className="rounded-2xl border border-forest-100 bg-forest-50/60 p-5">
      ...
      <ul className="mt-4 space-y-2">
        {contractor.verificationChecklist.map((item) => (
      ...
      {expanded && (
        <dl className="mt-4 space-y-2 border-t border-forest-600/10 pt-4">
          {contractor.verificationDetails.map((row) => (
...
function AboutSection({ contractor }) {
  return (
    <div className="rounded-2xl border border-ink-900/10 bg-white p-6">
      ...
      <p className="mt-3 text-sm leading-relaxed text-ink-700">{contractor.about}</p>
...
function ContactCard({ contractor, slug }) {
  ...
  const mailHref = `mailto:${contractor.email}`;
  ...
  return (
    <div className="rounded-2xl border border-ink-900/10 bg-white p-5">
      ...
      <a href={telHref} className="-mx-2 mt-4 flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-mist-100">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-ink-900">{contractor.phone}</p>
      ...
      <a href={mailHref} className="-mx-2 mt-1 flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-mist-100">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-ink-900">{contractor.email}</p>
  ...
  return (
    <div className="rounded-2xl border border-ink-900/10 bg-white p-5">
      ...
      <dl className="mt-2 divide-y divide-ink-900/5">
        {contractor.performance.map((row) => (
        ...
        {expanded &&
          contractor.performanceDetails.map((row) => (
            <div key={row.label} className="flex items-center justify-between gap-4 py-2 text-sm">
              <dt className="text-ink-500">{row.label}</dt>
              <dd className="font-semibold text-ink-900">{row.value}</dd>
            </div>
...
function ReviewsSection({ contractor }) {
  ...
  const total = contractor.ratingBreakdown.reduce((sum, row) => sum + row.count, 0) || 1;
  const visibleReviews = showAll ? contractor.reviews : contractor.reviews.slice(0, 3);
  const hasMore = contractor.reviews.length > 3;
    ...
    <div className="rounded-2xl border border-ink-900/10 bg-white p-6">
      ...
      <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-center">
        <div className="text-center sm:w-32 sm:shrink-0">
          <p className="text-4xl font-extrabold text-ink-900">{contractor.rating}</p>
          <StarRating value={contractor.rating} className="mt-1 justify-center" />
          <p className="mt-1 text-xs text-ink-500">({contractor.reviewCount} Reviews)</p>
        </div>
        <div className="flex-1 space-y-1.5">
          {contractor.ratingBreakdown.map((row) => (
...
function BadgesCard({ contractor }) {
  const [showAll, setShowAll] = useState(false);
  const visibleBadges = showAll ? contractor.badges : contractor.badges.slice(0, 3);
  const hasMore = contractor.badges.length > 3;
